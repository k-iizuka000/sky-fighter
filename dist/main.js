...
export class Game {
    constructor() {
        ...
        // ステージ管理
        this.currentStage = 1;
        this.totalStages = GAME_CONFIG.stages.total;
        this.enemiesKilled = 0;
        this.enemiesNeededForBoss = GAME_CONFIG.stages.enemiesNeededForBoss;
    ...
    spawnEnemies() {
        ...
        if (this.enemySpawnTimer > spawnRate) {
            // ステージに応じた敵を生成
            this.enemies.push(new Enemy(GAME_CONFIG.canvas.width, Math.random() * (GAME_CONFIG.canvas.height - 100) + 50, this.currentStage));
            this.enemySpawnTimer = 0;
        }
    }
    spawnBoss() {
    ...
    spawnPowerUps() {
        ...
        if (this.powerUpSpawnTimer > 400) {
            const types = ['double', 'triple', 'shield', 'beam', 'speed', 'rapid', 'life', 'bomb', 'megabomb'];
    ...
    activateMegaBomb() {
        if (this.megaBombs <= 0 || this.megaBombEffect.active)
            return;
        this.megaBombs--;
    ...
    addCombo() {
        ...
        // 5コンボごとに倍率+0.2、最大5倍
        this.combo.multiplier = Math.min(1.0 + Math.floor((this.combo.count - 1) / 5) * 0.2, GAME_CONFIG.combo.maxMultiplier);
    }
    resetCombo() {
        this.combo.count = 0;
    ...
    showStageClear() {
        ...
        if (clearedStageElement)
            clearedStageElement.textContent = this.currentStage.toString();
        if (stageClearBonusElement)
            stageClearBonusElement.textContent = stageBonus.toLocaleString();
        // 次のステージ情報
        const nextStage = this.currentStage + 1;
        const nextBossConfig = GAME_CONFIG.stages.bosses.find(boss => boss.stage === nextStage);
        if (nextStageNumberElement)
            nextStageNumberElement.textContent = nextStage.toString();
    ...
    updateUI() {
        ...
        if (megaBombsElement)
            megaBombsElement.textContent = this.megaBombs.toString();
        const weaponNames = {
            'normal': '通常弾',
            'double': 'ダブルショット',
            'triple': 'トリプルショット'
        ...
        if (weaponElement)
            weaponElement.textContent = weaponNames[this.player.weapon];
        // コンボ表示の更新
        if (comboElement) {
    ...
    updateBuffDisplay() {
        ...
        for (const buffName in this.player.buffs) {
            const buff = this.player.buffs[buffName];
            if (buff.active) {
                const div = document.createElement('div');
                div.className = 'buff-item';
                const seconds = Math.ceil(buff.timer / 60);
                div.textContent = `${buffNames[buffName]} (${seconds}s)`;
    ...
    updateStageUI() {
        ...
        if (currentStageElement)
            currentStageElement.textContent = this.currentStage.toString();
    ...
    hideBossUI() {
        ...
        const bossHpBarElement = document.getElementById('bossHpBar');
        if (bossNameElement)
            bossNameElement.classList.add('hidden');
        if (bossHpBarElement)
            bossHpBarElement.classList.add('hidden');
    }
    updateBossHp() {
    ...
    addButtonEvents(button, callback) {
        // clickイベント
    ...
    setupMobileControls() {
        ...
        // タッチイベントのヘルパー関数
        const addTouchEvents = (element, onStart, onEnd) => {
            if (!element)
                return;
        ...
        // 方向パッドのイベント設定
        addTouchEvents(dpadUp, () => this.touchControls.up = true, () => this.touchControls.up = false);
        addTouchEvents(dpadDown, () => this.touchControls.down = true, () => this.touchControls.down = false);
        addTouchEvents(dpadLeft, () => this.touchControls.left = true, () => this.touchControls.left = false);
        addTouchEvents(dpadRight, () => this.touchControls.right = true, () => this.touchControls.right = false);
        // アクションボタンのイベント設定
        addTouchEvents(shootButton, () => this.touchControls.shoot = true, () => this.touchControls.shoot = false);
        addTouchEvents(bombButton, () => this.touchControls.bomb = true, () => this.touchControls.bomb = false);
        // デフォルトのタッチ動作を無効化
        document.addEventListener('touchstart', (e) => {
            if (e.target && e.target.closest('#mobileControls')) {
                e.preventDefault();
            }
        }, { passive: false });

...
export class Player extends GameObject {
    constructor(x, y) {
        ...
        this.baseFireRate = GAME_CONFIG.player.baseFireRate;
        this.fireCounter = 0;
        this.shieldEffect = 0;
    ...
    render(ctx) {
        ...
        if (this.buffs.shield.active) {
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(this.position.x + this.width / 2, this.position.y + this.height / 2, this.width / 2 + 10, 0, Math.PI * 2);
            ctx.stroke();
            ctx.restore();
        }
    ...
    activateBuff(buffType, duration = 300) {
